...
public class DBmanager {
    ...
    public void add_user(Map m1){
        ...
        try {
            PreparedStatement ps1 = conn.prepareStatement("select * from user_info where name = ?");
    ...
    public List <String> fuzzy_query_item(Map m1) {
        /**
            ...
            * @api Description:  在 workout_item 中模糊查询用户输入的 item  ， 若存在一样的item 返回null , 不一致返回一个模糊查询得到的列表
            * @api Param [m1]:
    ...
    public Map return_Pie_data() throws Exception{
        /**
            * @api 成员函数描述  Author : PKY
            * @api return_Pie_data
        ...
        PreparedStatement pstmt = conn.prepareStatement("SELECT item , group_num , times_num from workout_data");     // 查询当前用户所有健身记录，计算出对应健身项目的锻炼次数，存为字典
        ...
        ResultSet item_muscle = pstmt.executeQuery();
    ...
    public List<Integer> return_Line_data() throws Exception{
        /**
            * @api 成员函数描述  Author : PKY
        ...
        PreparedStatement pstmt = conn.prepareStatement("SELECT date from workout_data");     // 查询当前用户所有健身记录，计算出对应健身项目的锻炼次数，存为字典
